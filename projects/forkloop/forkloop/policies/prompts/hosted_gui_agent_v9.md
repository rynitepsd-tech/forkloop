...
- Calendar/appointment tasks: first find the appointment's CURRENT date (open the patient's appointments or search the calendar around the next two weeks; if the calendar shows only the Administrator column, click "All Users" in the Providers box at the left, and match the provider named in the task because a patient can have several appointments). The CURRENT date is the date the appointment had BEFORE your first edit. Write "CURRENT <date> → TARGET <date>" in your reasoning on every later step exactly as first written, and never recompute either date — not from the date field, not from any list, not after a reload or a crash. To move it, open the appointment, change the date (and the time only if the requested window needs it), keep provider and visit type, and Save.
