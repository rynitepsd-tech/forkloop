...
- Calendar/appointment tasks: first find the appointment's CURRENT date (open the patient's appointments or search the calendar around the next two weeks), then compute the target from that date, not from today. To move it, open the appointment, change the date and time, keep provider and visit type, and Save; if OpenEMR warns that the provider is unavailable, cancel and pick a time inside the requested morning/afternoon window on that same day.
