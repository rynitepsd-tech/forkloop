...
- **What the test account can do (measured 2026-09-01, Free plan).** Desktops are 402-gated; use `FORKLOOP_SOLARI_KIND=sandbox` (headless: full controller loop, no agent channel). `revert()` returns 409 `Not revertable` and *destroyed a running sandbox* — never call it on a machine you need until Solari confirms the semantics; use pool `mode="fork"` (≈ 19 s p50). `disk_gb` is not honoured (4 GB disk; golden image ~89% full). The key lives in `~/.config/forkloop/env` (never in the repo); the golden snapshot id is `FORKLOOP_GOLDEN_SNAPSHOT_CLAIMS_OPS_V1` there. `forkloop build-world --attach <id>` resumes a failed build on the same machine; `forkloop reap` kills leftovers (the plan allows one machine at a time).
- **Desktop facts (Starter, 2026-09-02).** Session user is `desktop` on Xvfb `:0` (Chrome must run as that user; `runuser -u desktop -- env DISPLAY=:0 HOME=/home/desktop XDG_RUNTIME_DIR=/run/desktop`). Chords are one xdotool string (`"ctrl+a"`); the backend joins them. Click the omnibox at (640, 90) before typing a URL. Golden desktop snapshot: `FORKLOOP_GOLDEN_SNAPSHOT_CLAIMS_OPS_V1=snap_dl4e05ciyt1p` (v4); its parent `snap_dl4driq97904` cannot be deleted while v4 exists. The OpenEMR session is not in the snapshot: that family starts on the login page. Portal field positions (1280×720): login user (340,309) pass (340,391) button (90,449); appeal reason (340,382) auth (340,464) narrative (340,595). OpenEMR login user (706,422) pass (684,476) button (640,585).
