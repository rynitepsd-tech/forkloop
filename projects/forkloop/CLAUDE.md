...
forkloop compare --config configs/offline-comparison.yaml --out runs/<new-dir>   # offline A/B end to end
```
...
- **Solari SDK facts** (verified against solari-sandbox 0.2.0): desktops with `from_snapshot` come from `SandboxClient.create_desktop`, not `DesktopClient.create`; `commands.run` is argv, not shell; `kill()` destroys, `close()` only drops the channel; after `revert()` the guest briefly accepts one control connection, so the backend reconnects and re-polls `health()`. The PyPI package literally named `solari` is unrelated; depend on `solari-sandbox` and `solari-desktop`. **`solari-sandbox` 0.2.2 (2026-09-23) is broken**: `create_desktop` starts a billable desktop, then raises (`SessionHooks` has no `park`) before returning a handle; use 0.2.1. On a 0.2.1 desktop, `keyboard.hotkey("ctrl","a")` typed a plain `a` (use `press("ctrl+a")`) and `process.list()` returned empty names.
- **What the test account can do (measured 2026-09-01, Free plan).** Desktops are 402-gated; use `FORKLOOP_SOLARI_KIND=sandbox` (headless: full controller loop, no agent channel). `revert()` returned 409 `Not revertable` and *destroyed a running sandbox* until Solari fixed it on 2026-09-03 (see the desktop facts below; a refused revert now leaves the machine alive; the pool still falls back to fork when a revert is refused, logging `revert_unsupported_fell_back_to_fork`). `disk_gb` is not honoured (4 GB disk; golden image ~89% full). The key lives in `~/.config/forkloop/env` (never in the repo); the golden snapshot id is `FORKLOOP_GOLDEN_SNAPSHOT_CLAIMS_OPS_V1` there. `forkloop build-world --attach <id>` resumes a failed build on the same machine; `forkloop reap` kills leftovers (the plan allows one machine at a time).
- **Desktop facts (Starter, 2026-09-02).** Session user is `desktop` on Xvfb `:0` (Chrome must run as that user; `runuser -u desktop -- env DISPLAY=:0 HOME=/home/desktop XDG_RUNTIME_DIR=/run/desktop`). Chords are one xdotool string (`"ctrl+a"`); the backend joins them. Click the omnibox at (640, 90) before typing a URL. Golden desktop snapshot id lives in `~/.config/forkloop/env` (`FORKLOOP_GOLDEN_SNAPSHOT_CLAIMS_OPS_V1`; the Sept 15 golden `snap_dlft9omnpkyw` is the only snapshot left — v5, its ancestors and the sandbox golden were deleted on 2026-09-23). A fork with a full disk breaks OpenEMR ("table 'log' is full"); keep the image lean. The OpenEMR session is not in the snapshot: that family starts on the login page. On the Sept 15 golden every reset relaunches Chrome (stale leak-detection policy) and that loses the portal session; `open_initial_screen` logs back in as `agent`/`agent` and fails the reset if no logged-in portal screen is confirmed (2026-09-23: without it, all 28 cells of `runs/image-detail-live-20260923` opened on the portal login page). Check step-0 screenshots before trusting a tie. Chrome must run with `--disable-gpu` (no GPU process on the template; without it every authenticated OpenEMR page is an "Aw, Snap" renderer crash); `browser_setup.sh` starts it that way and `ClaimsOpsWorld.ensure_chrome_gpu_flag` (the `before_episode` stage) relaunches Chrome on older goldens, so v5 still works at ~8 s extra per reset. OpenEMR's calendar shows only the logged-in user's column until "All Users" in the Providers box (≈(63,567)) is clicked. **Since 2026-09-03 `revert()` and `snapshot()` on forks both work** (`docs/spikes.md`): `revert(golden)` 10/10 on one machine id, p50 100.9 s vs fork 92.0 s — both bimodal (≈ 22 s or 70–160 s, host-side restore, not our backoff) — so revert is the pool's reset mode (CLI default, fork fallback kept) and `best_of_n --search-mode fork` runs for real (family-3 seed 0 verified through a branch point). Branch pools are created with `reap_orphans_enabled=False` and closed after each branch; never let a second pool reap on an account where a live pool is running. Portal field positions (1280×720): login user (340,309) pass (340,391) button (90,449); appeal reason (340,382) auth (340,464) narrative (340,595). OpenEMR login user (706,422) pass (684,476) button (640,585).
...
- **Unscored is not failed.** `ORACLE_ERROR`, `INFRA_ERROR` and missing verdicts are excluded from every rate (`metrics.UNSCORED_REASONS`); a backend/transport exception while applying an action is `backend failed:` (infrastructure), a content rejection is `apply failed:` (the policy's invalid action).
...
- **Anthropic API code**: the key in `~/.config/forkloop/env` is identity-linked, so `ANTHROPIC_WORKSPACE_ID` must be exported alongside it (`teacher.anthropic_default_headers`). Uses the official `anthropic` SDK, model `claude-opus-5` by default, the GA `computer_toolset_20260801` (no beta header), adaptive thinking (do not set `budget_tokens`), and server-side refusal fallbacks (`fallbacks="default"`, beta `server-side-fallback-2026-07-01`) — `forkloop/policies/teacher.py`. The student policy talks to any OpenAI-compatible vLLM endpoint through `httpx` and never imports `openai`.
