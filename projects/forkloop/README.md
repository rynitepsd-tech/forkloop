...
Chart 2 has its two headline bars measured on the live golden (`bench/chart2_solari_0903.png`, 2026-09-03: `revert()` p50 100.9 s and `create(from_snapshot)` p50 92.0 s, n=10 each, 0 failures; both bimodal at ≈ 22 s or 70–160 s because the host-side restore of the 8.5 GB snapshot is — `bench/restore_bimodality_0903.png`; local and cold bars unmeasured). Chart 1 does not exist yet; `train/examples/` holds *synthetic placeholders* so the plotting pipeline can be checked, and their titles say so. The teacher has run for real: `claude-opus-5` verifies 11/17 family-3 episodes (64.7 % [41.3, 82.7]) through the live desktop world (`runs/teacher-*`, 2026-09-02; $2.22 per episode, $4.00 per verified episode, of which the desktop is about one cent — `docs/spikes.md`). Every failure is the 60-action budget, mostly spent recovering from Chrome tab crashes. OpenAI's GPT-5.6 Luna, through the OpenAI-compatible policy path with a hosted-agent prompt and a 120-action budget, verifies 17/20 seeds (85 % [64, 95]) at $0.07 per verified trajectory after five prompt iterations — 48× cheaper than Opus and ahead of it on shared seeds (12 vs 9) (`docs/spikes.md`, `runs/exports/opus_vs_luna.md`). Everything that needs a GPU is built and tested offline but has not been run for real. `docs/limitations.md` is the honest list.
...
| Teacher policy (Anthropic computer-use toolset) and student policy (vLLM, Fara/Qwen/UI-TARS output formats) | teacher **verified live**: 11/17 family-3 episodes at reward 1.0, seeds 0–14 (`docs/spikes.md`); student not run against a live endpoint |
| Training ladder (`train/`): SFT export, LoRA, held-out eval with Wilson CIs, plots, bake-off | built; no GPU run yet |
| Reset benchmark + local docker-compose baseline + cost model | **revert and fork measured on the golden desktop (2026-09-03)**: revert p50 100.9 s / p95 151.7 s, fork p50 92.0 s / p95 169.4 s, 0/10 failures each, bimodal restores; earlier fork-only runs: sandbox p50 19.1 s, desktop p50 25.0 s; local and cold bars not run |
| Day-1 spikes | run on Free then Starter: `revert()` was 409 on sandboxes *and* desktops until Solari fixed it on 2026-09-03 (now the pool's reset mode); fork restores RAM + windows; screenshot 0.13 s, click 0.19 s, observe-act-observe 0.45 s p50; snapshot 14–22 s — `docs/spikes.md` |
