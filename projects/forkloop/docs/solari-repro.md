...
| 5. Snapshot lineage cannot be flattened | `delete_snapshot(parent)` while a child exists | `./venv/bin/python -c "import asyncio; from forkloop.backends.solari import SolariBackend as B; b=B(); print(asyncio.run(b.delete_snapshot('snap_dl4driq97904')))"` → 409 while `snap_dl4e05ciyt1p` exists | `docs/archive/HANDOFF.md` |
