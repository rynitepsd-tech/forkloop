...
**Overnight totals (2026-09-04, seven runs, `docs/archive/overnight-2026-09-04.md`):** 121 seed-runs, 146 attempts, 96 verified,
...
| 3 resolve_denial, **student probes, base Fara1.5-4B + `--nav-macro`, seeds 200–204, 60 steps** (2026-09-05, `runs/fara15-4b-probe-{a-note,b-prompt,c-notes,ab}-f3-s200-204`, `docs/archive/student-2026-09-06.md`) | — | 0/5 each (one attempt) | 0/5 | $0.04–0.05 VM per probe. **OpenEMR login rung** (`verdict.details.ui_milestones`): (a) `--instruction-note` spelling out username/password fields **5/5**, (b) `fara_no_user_v1.md` prompt (no critical points, v5 conventions) **4/5** with 0 `ask_user_question` stops, (c) `--history-notes` 1/5 (retypes `pass` into the same field), (a)+(b) **5/5**. No probe reached the patient chart: after the login the model navigates by invented URLs (`/openemr/patient/1`) and drops the session |
